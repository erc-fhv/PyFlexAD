...
from pyflexad.optimization.centralized_power_controller_pyomo import CentralizedPowerControllerPyomo
...
class TestPeakShaving:
    ...
    def test_centralized_pyomo(self, seed: int, d, n, algorithm) -> None:
        ...
        co = CentralizedPowerControllerPyomo(power_demand=hh_data.power_demand)
        power_centralized = co.optimize(esr_list)

        # Todo add VertexBasePowerControllerPyomo
        agg = Aggregator.from_physical(esr_list, signal_vectors=SignalVectors.new(d), algorithm=algorithm)
        dco = VertexBasedPowerController(power_demand=hh_data.power_demand)
        power_decentralized, _ = dco.solve(agg.get_vertices())
