...
def main(d_list: list = (2, 4, 8, 16), n_flexibilities_list: list = (2, 4, 8, 16),
    ...
    for i, d in enumerate(d_list):
        for j, n_flexibilities in enumerate(n_flexibilities_list):
            ...
            dc_opt = VertexBasedCostController(power_demand=hh_data.power_demand, energy_prices=energy_prices, dt=dt)
            c_opt = CentralizedCostController(power_demand=hh_data.power_demand, energy_prices=energy_prices, dt=dt)
    ...
    """plot"""
    benchmark.show(optimizer_name="Cost optimization",
...
if __name__ == '__main__':
    bm = main(d_list=[24, 48, 72, 96],  # [96, ]
              n_flexibilities_list=[200, 600, 1000],  # [50, 100, 250, 500, 750, 1000],
    ...
    bm.to_pickle(os.path.join(FileUtils.data_dir, "paper_04", "test_V07.pickle"))

    bm = main(d_list=[24, 48, 72, 96],  # [96, ]
              n_flexibilities_list=[200, ],  # [50, 100, 250, 500, 750, 1000],
              algorithms=(Algorithms.LPVG_GUROBIPY,),
              parallelize=True,
              plot_3d_upr=True, plot_3d_time=True, plot_barchart_upr=True, plot_barchart_time=True)
    bm.to_pickle(os.path.join(FileUtils.data_dir, "paper_04", "test_V07_lpvg_gurobi.pickle"))
